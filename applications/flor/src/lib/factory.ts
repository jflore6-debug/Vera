...
export const TEMPLATE_LAYOUTS: { id: string; name: string; build: () => Slide }[] = [
  {
    ...
    build: () =>
      makeSlide({
        background: '#ffffff',
        elements: [
          makeText({ x: 8, y: 32, w: 84, h: 20, content: 'Presentation title', fontSize: 44, align: 'center', bold: true }),
          makeText({ x: 8, y: 54, w: 84, h: 10, content: 'Subtitle goes here', fontSize: 20, align: 'center', color: '#6b7280' }),
        ],
      }),
  },
  {
    ...
    build: () =>
      makeSlide({
        background: '#ffffff',
        elements: [
          makeText({ x: 6, y: 6, w: 88, h: 12, content: 'Slide title', fontSize: 32, bold: true }),
          makeText({
            x: 6,
            y: 22,
            w: 88,
  ...
  },
  {
    ...
    build: () =>
      makeSlide({
        ...
        elements: [
          makeText({ x: 6, y: 6, w: 88, h: 12, content: 'Slide title', fontSize: 32, bold: true }),
          makeText({ x: 6, y: 22, w: 42, h: 65, content: 'Left column text', fontSize: 18, bullet: true }),
          makeText({ x: 52, y: 22, w: 42, h: 65, content: 'Right column text', fontSize: 18, bullet: true }),
        ],
      }),
  },
  {
    ...
    build: () =>
      makeSlide({
        background: '#ffffff',
        elements: [
          makeText({ x: 6, y: 6, w: 44, h: 12, content: 'Slide title', fontSize: 28, bold: true }),
          makeText({ x: 6, y: 22, w: 44, h: 65, content: 'Supporting text goes here.', fontSize: 18 }),
    ...
    build: () =>
      makeSlide({
        ...
        elements: [
          ...
          makeText({ x: 10, y: 70, w: 80, h: 10, content: '— Attribution', fontSize: 18, align: 'center', color: '#9aa0ac' }),
        ],
      }),
  },
  {
